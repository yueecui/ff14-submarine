/**
 * 通过图片文件名，获取wiki图片的上传地址
 * @param filename 上传的文件名，需要包括后缀
 * @returns wiki图片的上传地址
 */
...
export function getImageUrl(filename: string) {
    ...
    if (window.mw) {
        ...
        return [`/${[pkg.wikiPrefix]}/uploads`, hex[0], hex[0] + hex[1], filename].join('/');
    }
}

export function getFF14IconUrl(icon_id: number) {
    return getImageUrl(icon_id.toString().padStart(6, '0') + '.png');
}
