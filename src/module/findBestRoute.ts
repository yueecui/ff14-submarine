export function findBestRoute(
    ...
    wayPoints: Record<number, Record<string, any>>,
) {
    ...
    for (let j = 0; j < trees.length; j++) {
        ...
        for (let i = 0; i < tree.length; i++) {
            ...
            temp_ms =
                temp_ms > wayPoints[tree[i]].statReq.surveillance.high
                    ? temp_ms
                    : wayPoints[tree[i]].statReq.surveillance.high;
            temp_mr =
                temp_mr > wayPoints[tree[i]].statReq.retrieval.optim
                    ? temp_mr
                    : wayPoints[tree[i]].statReq.retrieval.optim;
    ...
    return {
        routeOrder: route,
        routeCode: route.map((wp_id) => {
            return wayPoints[wp_id].code;
        }),
        time: time,
        distance: distance,
        max_surveillance: ms,
...
// 给定一个数组，生成所有的组合
function loopTree(ary: number[]): number[][] {
    const r = [] as number[][];
    if (ary.length > 1) {
